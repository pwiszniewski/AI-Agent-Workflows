...
# embeddings = [get_embedding(doc) for doc in documents] # dont run unnecesary
ids = [f"id{i}" for i in range(len(documents))]

#create chroma database client
# chroma_client = chromadb.Client()
chroma_client = chromadb.PersistentClient(path="./my_code/ch06/chroma_db")
...
collection = chroma_client.get_or_create_collection(name="exercise_docs")
print(collection)
# exit()
