...
import chromadb
import tiktoken
from agents import Agent, Runner, function_tool, trace  # OpenAI Agents SDK
...
results.
cite at least two distinct references for every answer
...
queries = [
    ...
    # "time machine",
    # "DeLorean",
    "At what exact time does the lightning strike the clock tower?",
]
...
for query in queries:
    ...
    print(f"QUERY: {query}")
    print("=" * 60)
    with trace("Chapter 6 Hybrid RAG Demo"):
        result = Runner.run_sync(agent, query)
        print("\n--- ANSWER ---\n", result.final_output)

#############################################################################
# ============================================================
# QUERY: At what exact time does the lightning strike the clock tower?
# ============================================================

# --- ANSWER ---
#  Based on the script provided, there is no mention of a lightning strike hitting a clock tower at a specific time.

# The search results for the script provided discuss an atomic bomb test at a site with a tower [Reference 1, 2], and various scenes involving clocks and time [Reference 1, 2, 3], but they do not contain the plot point regarding the lightning striking the clock tower at 10:04.